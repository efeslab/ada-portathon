...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  //   calcOptFlowLKPyrWInitMex2(Ipyr, Jpyr, featurePnt, winSize, accuracy_th, max_iter, initFPnt)
  ...
  const int *nFeatures;
  double *iP[MAX_LEVEL], *jP[MAX_LEVEL];
  ...
  accuracy_th=(double)mxGetScalar(prhs[4]);
  max_iter=(int)mxGetScalar(prhs[5]);
  initFPnt=(double*)mxGetPr(prhs[6]);
	
  winSize = (int)mxGetScalar(prhs[3]);
  fPnt=(double*)mxGetPr(prhs[2]);
  nFeatures=mxGetDimensions(prhs[2]);
  ...
  for(int i=0; i<cellDims[0]; i++){
  	...
  	jP[i]= (double*)mxGetPr(mxGetCell(prhs[1],i));
  }
  ...
  for(int i=0; i<nFeatures[1]; i++){
	...
	initFPnt[i*2]=initFPnt[i*2]-1;
	initFPnt[i*2+1]=initFPnt[i*2+1]-1;
	valid[i]=1;
  }
  calcPyrLKTrackWInit( iP, jP, imdims, cellDims[0], fPnt, nFeatures[1], winSize, 
	 newFPnt, initFPnt, valid, accuracy_th, max_iter); 
  //idx convert from matlab to c
  for(int i=0; i<nFeatures[1]*2; i++){
	fPnt[i]=fPnt[i]+1;
	initFPnt[i]=initFPnt[i]+1;
