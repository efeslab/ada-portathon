...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  //   calcOptFlowPyrLKMex(Ipyr, IdxPyr, IdyPyr, Jpyr, featurePnt, winSize, accuracy_th, max_iter)
  ...
  //iP=(double**)malloc(sizeof(double*)*cellDims[0]);
  //iDxP=(double**)malloc(sizeof(double*)*cellDims[0]);
  //iDyP=(double**)malloc(sizeof(double*)*cellDims[0]);
  //jP=(double**)malloc(sizeof(double*)*cellDims[0]);
  ...
  if(nrhs>6){
	calcPyrLKTrack( iP, iDxP, iDyP, jP, imdims, cellDims[0], fPnt, nFeatures[1], winSize, 
